 ...
 * PUT  ?resource=depots&id=N
 * PUT  ?resource=assignments&id=N
 * PUT  ?resource=drivers&id=N
...
if ($resource === 'vehicles') {
    ...
    if ($method === 'PUT' && $id) {
        ...
        $b = readBody();
        $pdo->prepare("UPDATE Vehicles SET RegistrationNumber=:reg, CategoryID=:cat,
            Model=:model, Manufacturer=:mfr, YearOfManufacture=:yr,
            CurrentOdometerReading=:odo, DepotID=:depot, OperationalStatus=:status
            WHERE VehicleID=:id")->execute([
            ':reg'=>$b['RegistrationNumber'],':cat'=>$b['CategoryID'],
            ':model'=>$b['Model'],':mfr'=>$b['Manufacturer'],':yr'=>$b['YearOfManufacture'],
            ':odo'=>$b['CurrentOdometerReading']??0,':depot'=>$b['DepotID'],
            ':status'=>$b['OperationalStatus'],':id'=>$id,
        ]);
        jsonOk(['updated' => $id]);
    }
    if ($method === 'DELETE' && $id) {
...
if ($resource === 'assignments') {
    ...
    if ($method === 'PUT' && $id) {
        ...
        $b = readBody();
        $pdo->prepare("UPDATE VehicleAssignments SET VehicleID=:v,DriverID=:d,
            StartDate=:s,EndDate=:e,IsPermanent=:p,DepotID=:dep WHERE AssignmentID=:id")
            ->execute([':v'=>$b['VehicleID'],':d'=>$b['DriverID'],':s'=>$b['StartDate'],
...
if ($resource === 'drivers') {
    ...
    if ($method === 'PUT' && $id) {
        ...
        $b = readBody();
        $pdo->prepare("UPDATE Drivers SET FirstName=:fn,LastName=:ln,ContactInformation=:ci,
            DepotID=:dep,LicenceType=:lt,LicenceExpiryDate=:led,
            EmploymentStatus=:es,EmergencyContactDetails=:ec WHERE DriverID=:id")->execute([
            ':fn'=>$b['FirstName'],':ln'=>$b['LastName'],':ci'=>$b['ContactInformation']??null,
            ':dep'=>$b['DepotID'],':lt'=>$b['LicenceType'],':led'=>$b['LicenceExpiryDate'],
...
if ($resource === 'mechanics') {
    ...
    if ($method === 'GET') {
        $rows = $pdo->query("
            ...
            ORDER BY m.LastName
        ")->fetchAll();
        // Split cert string back to array
        foreach ($rows as &$r) {
...
if ($resource === 'vehicle_status_breakdown') {
    ...
    $rows = $pdo->query("
        ...
        FROM Vehicles GROUP BY OperationalStatus
    ")->fetchAll();
    jsonOk($rows);
}
